...
import { useConversationState } from '../core/useConversationState';
  ...
  const handleSend = useCallback(
    async (text: string) => {
      ...
      setConversations(prev =>
        prev.map(c =>
          c.id === activeId ? { ...c, messages: [...c.messages, user] } : c
        )
      );
      setIsTyping(true);
      setQuickReplies([]);
      try {
        ...
        const flowId =
          conversations.find(c => c.id === activeId)?.flowId || 'about';
