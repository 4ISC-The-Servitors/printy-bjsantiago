...
const Filter: React.FC<FilterProps> = ({
  ...
  return (
    <div className={`${floating ? 'relative' : 'space-y-3'} ${className ?? ''}`}>
      ...
      <div className={`flex items-center justify-center`}>
        ...
        >
          <SlidersHorizontal className={`w-3 h-3 sm:w-4 sm:h-4 md:w-5 md:h-5 ${isMobile ? 'mr-1' : 'mr-2'}`} />
      ...
      {showResultCount && hasFilters && (
        <div className={textClasses.body}>
          Found <span className="font-semibold text-neutral-900">{resultCount}</span>{' '}
      ...
      {isExpanded && (
        <div className={`bg-white border border-neutral-200 rounded-xl shadow-lg ${floating ? 'p-3 sm:p-4 md:p-5 lg:p-6 space-y-3 sm:space-y-4 md:space-y-5' : spacingClasses.padding + ' ' + spacingClasses.gap} animate-in fade-in slide-in-from-top-2 duration-200 ${
          floating ? 'absolute top-full left-0 mt-2 w-72 sm:w-80 md:w-96 lg:w-[28rem] z-50' : ''
        }`}>
          ...
          {filterConfig.showDateRange && (
            <div className="space-y-2 sm:space-y-3">
              ...
              <div className={`flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3`}>
                ...
                <span className="text-neutral-400 text-xs sm:text-sm md:text-base text-center sm:text-left">to</span>
                <div className="flex-1">
                  <Input
                    type="date"
                    value={value.dateTo || ''}
                ...
                >
                  <span className="text-neutral-700">
                    {value.statuses?.length > 0 
                      ? filterConfig.statusOptions.find(opt => opt.value === value.statuses[0])?.label || 'Select status'
                      : 'Select status'
                    }
                  </span>
                  <ChevronDown className={`w-3 h-3 sm:w-4 sm:h-4 md:w-5 md:h-5 text-neutral-400 transition-transform ${isStatusDropdownOpen ? 'rotate-180' : ''}`} />
                </button>
                    ...
                    {filterConfig.statusOptions.map(opt => (
                      <button
                        ...
                        className={`w-full px-2 sm:px-3 md:px-4 py-1.5 sm:py-2 md:py-2.5 text-left text-xs sm:text-sm md:text-base hover:bg-neutral-50 ${
                          value.statuses?.includes(opt.value) ? 'bg-primary-50 text-primary-700' : 'text-neutral-700'
                        }`}
                      >
                        {opt.label}
                      </button>
          ...
          {filterConfig.showRoleFilter && filterConfig.roleOptions && filterConfig.roleOptions.length > 0 && (
            <div className="space-y-2 sm:space-y-3">
                      <div className={`text-xs sm:text-sm md:text-base lg:text-lg font-medium text-neutral-700`}>
                        {filterConfig.roleOptions[0]?.value.includes('role') ? 'Customer Role' :
                         'Role'}
                      </div>
                ...
                >
                  <span className="text-neutral-700">
                    {value.roles && value.roles.length > 0 
                      ? filterConfig.roleOptions!.find(opt => opt.value === value.roles![0])?.label || 'Select role'
                      : 'Select role'
                    }
                  </span>
                  <ChevronDown className={`w-3 h-3 sm:w-4 sm:h-4 md:w-5 md:h-5 text-neutral-400 transition-transform ${isRoleDropdownOpen ? 'rotate-180' : ''}`} />
                </button>
                  ...
                  <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-neutral-200 rounded-lg shadow-lg z-10 max-h-48 sm:max-h-56 md:max-h-64 overflow-y-auto">
                    {filterConfig.roleOptions!.map(opt => (
                      <button
                        ...
                        className={`w-full px-2 sm:px-3 md:px-4 py-1.5 sm:py-2 md:py-2.5 text-left text-xs sm:text-sm md:text-base hover:bg-neutral-50 ${
                          value.roles?.includes(opt.value) ? 'bg-primary-50 text-primary-700' : 'text-neutral-700'
                        }`}
                      >
                        {opt.label}
                      </button>
