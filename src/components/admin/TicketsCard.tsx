...
export type TogglePending = (item: { id: string; label: string; type: 'ticket' }, checked: boolean) => void;
...
const getBadgeVariantForStatus = (status: string): 'info' | 'warning' | 'secondary' => {
    ...
    <Card className="p-0" ref={containerRef as unknown as React.Ref<HTMLDivElement>}>
      ...
      <div className="flex items-center justify-end px-3 py-2 sm:px-4">
        <div className="flex items-center gap-2 text-neutral-500 text-xs">
          <Badge size="sm" variant="secondary">{mockTickets.length}</Badge>
          <Button
            variant="secondary"
            size="sm"
            onClick={() => setBulk(b => !b)}
      ...
      <div className="space-y-4 sm:space-y-6 px-3 sm:px-4 pb-3">
        {mockTickets.map(t => (
          <div key={t.id} className="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 lg:p-5 rounded-lg border bg-white/60 hover:bg-white transition-colors">
            {bulk && (
              <input
                type="checkbox"
                className="w-4 h-4 accent-brand-primary shrink-0"
                checked={isPending(t.id)}
                onChange={e => togglePending({ id: t.id, label: t.id, type: 'ticket' }, e.target.checked)}
                title="Select ticket"
              />
            )}
            ...
            <div className="flex-1 min-w-0 space-y-1 sm:space-y-2">
              ...
              <div className="flex justify-start sm:hidden">
                <Badge size="sm" variant={getBadgeVariantForStatus(t.status)} className="text-xs">
                  {t.status}
                </Badge>
              </div>
              <div className="hidden sm:flex sm:items-center sm:gap-3">
                <Badge size="sm" variant={getBadgeVariantForStatus(t.status)} className="text-xs sm:text-sm">
                  {t.status}
                </Badge>
              </div>
            </div>
