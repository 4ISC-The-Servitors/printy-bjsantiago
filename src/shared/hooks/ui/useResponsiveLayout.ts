...
export function useResponsiveLayout() {
  ...
  const { getStatusBadgeClasses, getPriorityBadgeClasses } =
    useResponsiveBadge();
  ...
  const getOrderCardLayout = useMemo(() => {
    return {
      ...
      chatButton:
        'h-7 px-3 text-xs sm:h-7 sm:px-2.5 sm:text-xs md:h-8 md:px-3 md:text-sm lg:h-9 lg:px-4 lg:text-base shrink-0',
  ...
  const getGridClasses = useMemo(
    () => ({
      ...
      cardGridSpacious:
        'grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-5 md:gap-6 lg:gap-8',
  ...
  const getContainerClasses = useMemo(
    () => ({
      ...
      modalContainerLarge:
        'max-w-lg sm:max-w-xl md:max-w-2xl lg:max-w-4xl mx-auto',
    }),
