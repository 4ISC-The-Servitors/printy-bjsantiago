 ...
 */
export async function fetchTicketForAdmin(params: ActionExecutionParams): Promise<ActionExecutionResult> {
  ...
  try {
    ...
    const { data: allMessages, error: messagesError } = await supabase
      .rpc('api_fetch_chat_messages_v2', { p_session_id: inquiry.session_id });
    ...
    const shouldShowFullHistory = inquiry.inquiry_status === 'pending_admin_reply' || inquiry.inquiry_status === 'resolved';
    ...
    if (allMessages && allMessages.length > 0) {
      if (shouldShowFullHistory) {
        ...
        const relevantMessages = allMessages.filter((msg: any) => 
        ...
        const descriptionMessage = allMessages.find((msg: any) => 
