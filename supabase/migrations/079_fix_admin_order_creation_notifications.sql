-- Migration 079: Fix admin order creation notifications
-- Admin-created orders should notify customers, not admins
...
COMMENT ON FUNCTION notify_order_events() IS 'Notifies only the OTHER party when orders are updated - prevents self-notifications. Fixed to handle admin-created orders properly.';
