-- Migration 076: Fix notify_ticket_events function to detect sender role and notify only the other party
...
COMMENT ON FUNCTION notify_ticket_events() IS 'Notifies only the OTHER party when tickets are updated - prevents self-notifications';
