  ...
  try {
    ...
    switch (action) {
      ...
      case 'propose-quote': {
        ...
        if (!quote_id || typeof quoted_price !== 'number')
          return {
            statusCode: 400,
            body: JSON.stringify({
              error: 'quote_id and quoted_price required',
            }),
          };
        const { error } = await supabase
          .from('quotes')
          .update({ quoted_price, notes, status: 'proposed' })
          .eq('id', quote_id);
        if (error)
          return {
            statusCode: 400,
      ...
      case 'send-for-approval': {
        ...
        if (!quote_id)
          return {
            statusCode: 400,
            body: JSON.stringify({ error: 'quote_id required' }),
          };
        const { error } = await supabase
          .from('quotes')
      ...
      case 'place-order': {
        ...
        if (!quote_id || typeof valued !== 'boolean')
          return {
            statusCode: 400,
            body: JSON.stringify({ error: 'quote_id and valued required' }),
          };
        const { error } = await supabase
          .from('quotes')
          .update({ status: valued ? 'ordered' : 'rejected' })
          .eq('id', quote_id);
        if (error)
          return {
            statusCode: 400,
